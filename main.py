...
def _rate(rows: list[dict], key: str, value: str) -> float:
    ...
    matches = sum(1 for row in rows if str(row.get(key, "")).strip().lower() == target)
...
def _summarize_trials(trials: list[dict]) -> dict[str, float | int]:
    ...
    rt_values = [_as_float(row.get("decision_rt", None)) for row in responded]
    rt_values = [value for value in rt_values if value is not None]
    mean_rt_ms = _mean(rt_values) * 1000.0 if rt_values else 0.0

    fusion_rate = _rate(incongruent_responded, "reported_syllable", "da") if incongruent_responded else 0.0
...
def run(options: TaskRunOptions):
    """Run McGurk Effect Task in human/qa/sim mode with one auditable flow."""
...
def main() -> None:
    ...
    options = parse_task_run_options(
        task_root=task_root,
        description="Run McGurk Effect Task in human/qa/sim mode.",
